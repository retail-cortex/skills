...
func main() {
	...
	apiDir := filepath.Join(workspaceRoot, "api", "v1")
	outDir := filepath.Join(workspaceRoot, "docs", "architecture")

	fmt.Println("Generating Mermaid architecture diagrams from api/v1/*.proto...")
	cmdGen := exec.Command(genBinary, "-d", apiDir, "-o", outDir, "-md", "-r=false")
	cmdGen.Stdout = os.Stdout
	cmdGen.Stderr = os.Stderr
	if err := cmdGen.Run(); err != nil {
		log.Fatalf("diagram generation failed: %v", err)
	}
