...
func TestExecute_Config(t *testing.T) {
	...
	// Test config set server
	code := Execute([]string{"config", "set", "server", "http://test-server:8080"}, out, errOut)
	assert.Equal(t, 0, code)
	assert.Contains(t, out.String(), "Updated server URL: http://test-server:8080")

	// Test config set api_key
	out2 := &bytes.Buffer{}
	errOut2 := &bytes.Buffer{}
	code = Execute([]string{"config", "set", "api_key", "secret-key-12345"}, out2, errOut2)
	assert.Equal(t, 0, code)
	assert.Contains(t, out2.String(), "Updated API key")

	// Test config show
	out3 := &bytes.Buffer{}
	errOut3 := &bytes.Buffer{}
	code = Execute([]string{"config", "show"}, out3, errOut3)
	assert.Equal(t, 0, code)
	assert.Contains(t, out3.String(), "http://test-server:8080")
...
func TestExecute_RegisterMissingArgs(t *testing.T) {
	out := &bytes.Buffer{}
	errOut := &bytes.Buffer{}
	code := Execute([]string{"register"}, out, errOut)
	assert.Equal(t, 1, code)
