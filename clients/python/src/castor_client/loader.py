...
def find_registry_root() -> Path:
    ...
    if "TEST_SRCDIR" in os.environ:
        ...
        for cand in candidates:
            if (cand / "clients").exists() or (cand / "cmd").exists() or (cand / "pkg").exists() or (cand / "skills").exists() or (cand / "examples").exists():
                return cand

        # Search for any SKILL.md under TEST_SRCDIR
        for cand in test_srcdir.rglob("SKILL.md"):
            for parent in [cand] + list(cand.parents):
                if (parent / "clients").exists() or (parent / "cmd").exists() or (parent / "pkg").exists() or (parent / "skills").exists() or (parent / "examples").exists():
                    return parent
...
def load_all_skills(
    ...
    # 1. Scan workspace packages and example skill packages for skills
    package_roots = [root / "packages", root / "examples"]
    if root.name in ("packages", "examples"):
        package_roots = [root]
    for pkg_parent in package_roots:
        if pkg_parent.is_dir():
            matched_dirs = set(pkg_parent.glob("skills-*/src/*/skills/*")).union(pkg_parent.glob("*/skills/src/*/skills/*"))
            for skill_dir in sorted(matched_dirs):
...
def load_skills_from_package(
    ...
    for parent_dir in search_dirs:
        if parent_dir.is_dir():
            matched_pkgs = set(parent_dir.glob("skills-*")).union(parent_dir.glob("*/skills"))
            for p in sorted(matched_pkgs):
                src_dir = p / "src"
                if src_dir.is_dir():
