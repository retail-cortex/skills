...
func (h *ServerHandlers) ListSkills(c *gin.Context) {
	...
	db := data.GetDB()
	res, err := h.castorService.ListSkills(db, query, pagination)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"detail": err.Error()})
		return
	}
...
func (h *ServerHandlers) GetSkill(c *gin.Context) {
	...
	db := data.GetDB()
	res, err := h.castorService.GetSkill(db, skillIDOrName)
	if err != nil {
...
func (h *ServerHandlers) RegisterSkill(c *gin.Context) {
	...
	db := data.GetDB()
	res, err := h.castorService.CreateSkill(db, authCtx.App.AppID, req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"detail": err.Error()})
		return
	}
...
func (h *ServerHandlers) ReplaceSkill(c *gin.Context) {
	...
	db := data.GetDB()
	res, err := h.castorService.UpdateSkill(db, skillID, authCtx.App.AppID, req, true)
	if err != nil {
		if errors.Is(err, data.ErrSkillNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"detail": err.Error()})
			return
...
func (h *ServerHandlers) UpdateSkill(c *gin.Context) {
	...
	db := data.GetDB()
	res, err := h.castorService.UpdateSkill(db, skillID, authCtx.App.AppID, req, false)
	if err != nil {
		if errors.Is(err, data.ErrSkillNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"detail": err.Error()})
			return
...
func (h *ServerHandlers) DeleteSkill(c *gin.Context) {
	...
	skillID := strings.TrimPrefix(c.Param("skill_id"), "/")
	db := data.GetDB()
	res, err := h.castorService.DeleteSkill(db, skillID, authCtx.App.AppID)
	if err != nil {
		if errors.Is(err, data.ErrSkillNotFound) {
			c.JSON(http.StatusNotFound, gin.H{"detail": err.Error()})
			return
