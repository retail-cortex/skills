...
`
	...
	result := AuditSkillDirectory(skillDir)
...
func TestAuditSkillDirectory_MissingSKILLMD(t *testing.T) {
	...
	result := AuditSkillDirectory(tmpDir)
...
`
	...
	summary := AuditAllSkills(tmpDir, true)
...
func TestAuditSkillDirectory_Failures(t *testing.T) {
	...
	// 1. Invalid frontmatter (invalid kebab name & missing license)
...
`
	...
	res := AuditSkillDirectory(badFMDir)
	assert.False(t, res.Passed)
	assert.False(t, res.FrontmatterValid)

	// 2. Missing references and examples
...
`
	...
	res2 := AuditSkillDirectory(noL3Dir)
